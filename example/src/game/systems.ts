...
export const paddleControl: System = (_w, ctx) => {
...
export const ballBrickCollision: System = (w, _ctx) => {
...
export const ballBounds: System = (_w, ctx) => {
  if (refs.ballId < 0) return;
  const s = gameStore.get();
...
export const ballSpeedClamp: System = (_w, _ctx) => {
  if (refs.ballId < 0) return;
  if (gameStore.get().status !== "playing") return;
...
export const inputHandler: System = (_w, ctx) => {
  let tapped = false;
  for (let i = 0; i < ctx.input.touches.length; i++) {
  ...
  if (s.status === "ready") {
    const info = buildLevel(screenW, screenH);
    gameStore.reset({
      status: "playing",
